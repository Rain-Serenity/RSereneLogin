package fun.ksnb.rserenelogin.velocity.main;
...
import fun.ksnb.rserenelogin.velocity.impl.ChatSessionHandler;
import fun.ksnb.rserenelogin.velocity.impl.NewChatSessionPacketIDEvent;
import fun.ksnb.rserenelogin.velocity.logger.Slf4jLoggerBridge;
...
public class RSereneLoginVelocity implements IPlugin {
   ...
   private final fun.ksnb.rserenelogin.velocity.impl.VelocityServer runServer;
   ...
   public RSereneLoginVelocity(ProxyServer server, Logger logger, @DataDirectory Path dataDirectory) {
      ...
      this.runServer = new fun.ksnb.rserenelogin.velocity.impl.VelocityServer(this.server);
   ...
   public void onInitialize(ProxyInitializeEvent event) {
      try {
         ...
         this.injector = (Injector)this.pluginLoader.findClass("moe.caa.rserenelogin.velocity.injector.VelocityInjector").getConstructor().newInstance();
      ...
      (new CommandHandler(this)).register("rslogin", "rsl");
      ...
      this.server.getEventManager().register(this, DisconnectEvent.class, PostOrder.LAST, (AwaitingEventExecutor<DisconnectEvent>) (disconnectEvent) -> disconnectEvent.getLoginStatus() == LoginStatus.CONFLICTING_LOGIN ? null : EventTask.async(() -> this.removePlayer(disconnectEvent.getPlayer())));
      this.server.getEventManager().register(this, NewChatSessionPacketIDEvent.class, (AwaitingEventExecutor<NewChatSessionPacketIDEvent>) (packetEvent) -> EventTask.withContinuation((continuation) -> {
             try {
   ...
   private void injectPlayer(Player player) {
      ...
      connectedPlayer.getConnection().getChannel().pipeline().addBefore("handler", "RSereneLoginChatSession", new ChatSessionHandler(player, this.server.getEventManager()));
   }
   ...
   @Generated
   public fun.ksnb.rserenelogin.velocity.impl.VelocityServer getRunServer() {
